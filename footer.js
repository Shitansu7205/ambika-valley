document.write(`

<footer class="footer-top-area footer-top-area-two ">
			<div class="container">
				
				<div class="footer-middle-area pt-60">
					<div class="row">
						<div class="col-lg-3 col-md-6">
							<div class="single-widget">
								<a href="index.html">
									<img src="assets/gulab-images/logo.png" alt="Image" style="height:85px;border-radius:10px;">
								</a>
								<ul class="social-icon">
									<li>
										<a href="#">
											<i class="bx bxl-facebook"></i>
										</a>
									</li>
									
									<li>
										<a href="#">
											<i class="bx bxl-instagram"></i>
										</a>
									</li>
								</ul>
							</div>
						</div>
						<div class="col-lg-3 col-md-6">
							<div class="single-widget">
								<h3>Quick Links</h3>
								<ul>
									<li>
										<a href="index.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Home
										</a>
									</li>
									<li>
										<a href="room.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Rooms
										</a>
									</li>
									<li>
										<a href="gallery.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											 Gallery
										</a>
									</li>
									
									
									<li>
										<a href="contact.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Contact 
										</a>
									</li>
									<li>
										<a href="gulabkothibygrapevinevaranasi">
											<i class="right-icon bx bx-chevrons-right"></i>
											Book Now 
										</a>
									</li>
								</ul>
							</div>
						</div>
						<div class="col-lg-3 col-md-6">
							<div class="single-widget">
								<h3>Services</h3>
								<ul>
									<li>
										<a href="services.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Free Wifi
										</a>
									</li>
									<li>
										<a href="services.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Air Conditioning
										</a>
									</li>
									<li>
										<a href="services.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Restaurant
										</a>
									</li>
									<li>
										<a href="services.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											Room Services
										</a>
									</li>
									<li>
										<a href="hotel-terms.html">
											<i class="right-icon bx bx-chevrons-right"></i>
											CCTV Security
										</a>
									</li>
									
									
								</ul>
							</div>
						</div>
						<div class="col-lg-3 col-md-6">
							<div class="single-widget">
								<h3>Contact Info</h3>
								<ul class="information">
									<li class="address">
										<i class="flaticon-maps-and-flags"></i>
										<span>Address</span>
										117/1, Parao Ramnagar Road, Ward, Mauja, Katesar, Chandauli, Uttar Pradesh, India
									</li>
									<li class="address">
										<i class="flaticon-call"></i>
										<span>Phone</span>
										<a href="tel:+917565004489">
										+91 75650 04489
										</a>
									</li>
									<li class="address">
										<i class="flaticon-envelope"></i>
										<span>Email</span>
										<a href="mailto:reservations.gk@grapevinevaranasi.com">
                      reservations.gk@grapevinevaranasi.com
										</a>
									</li>
								</ul>
							</div>
						</div>
					</div>
				</div>
				<div class="footer-bottom-area">
					<div class="row align-items-center">
						<div class="col-lg-6">
							<div class="copy-right">
								<p>Copyright @2026 | All Rights Reserved</p>
							</div>
						</div>
						<div class="col-lg-6">
							<div class="designed">
								<p>Designed & Developed By  <a href="https://bookingjini.com/" target="_blank"><img src="assets/img/logo-orange.png" alt="Bookingjini Logo" style="width:130px;"/></a></p>
							</div>
						</div>
					</div>
				</div>
			</div>
			<div class="footer-shape">
				<img src="https://www.hotelpahadi.com/assets/img/shape/white-shape-bottom.png" alt="Image">
			</div>
		</footer>
		<!-- Start Go Top Area -->
		<!-- <div class="go-top">
			<i class="bx bx-chevrons-up"></i>
			<i class="bx bx-chevrons-up bx-fade-up"></i>
		</div> -->
		<!-- End Go Top Area --> 

        
      <!-- Jquery Min JS -->
		  <!-- <script data-cfasync="false" src="../../cdn-cgi/scripts/5c5dd728/cloudflare-static/email-decode.min.js"></script> -->
		  <script src="assets/js/jquery.min.js"></script> 
		  <!-- Bootstrap Bundle Min JS -->
		  <script src="/assets/js/bootstrap.bundle.min.js"></script>
		  <!-- Meanmenu Min JS -->
		  <script src="/assets/js/meanmenu.min.js"></script>
		  <!-- Owl Carousel Min JS -->
		  <script src="/assets/js/owl.carousel.min.js"></script>
		  <!-- Wow Min JS -->
		  <script src="/assets/js/wow.min.js"></script>
		  <!-- Nice Select Min JS -->
		  <script src="/assets/js/nice-select.min.js"></script>
		  <!-- Magnific Popup Min JS -->
		  <script src="/assets/js/magnific-popup.min.js"></script>
		  <!-- Mixitup JS --> 
		  <script src="/assets/js/jquery.mixitup.min.js"></script>
		  <!-- Appear Min JS --> 
		  <script src="/assets/js/appear.min.js"></script>
		  <!-- Odometer Min JS --> 
		  <script src="/assets/js/odometer.min.js"></script>
		  <!-- Datepicker Min JS --> 
		  <script src="/assets/js/bootstrap-datepicker.min.js"></script>
		  <!-- Ofi Min JS --> 
		  <script src="/assets/js/ofi.min.js"></script>
		  <!-- jarallax Min JS --> 
		  <script src="/assets/js/jarallax.min.js"></script>
		  <!-- Form Validator Min JS -->
		  <script src="/assets/js/form-validator.min.js"></script>
		  <!-- Contact JS -->
		  <script src="/assets/js/contact-form-script.js"></script>
		  <!-- Ajaxchimp Min JS -->
		  <script src="assets/js/ajaxchimp.min.js"></script>
		  <!-- Custom JS -->
		  <script src="/assets/js/custom.js"></script>
		

 

        <!-- Start Instant Booking Widget-->
            <script>
            (function() {
                const addElement = (tag, attributes, parent = document.body) => {
                    const element = document.createElement(tag);
                    Object.entries(attributes).forEach(([key, value]) => element.setAttribute(key, value));
                    parent.appendChild(element);
                    return element;
                };

                const rootWidget = addElement("div", {
                    id:"Mw31A3qlpySYe9FG",
                    hotelId: 8812,
                });

                const script = addElement("script", {
                    src: "https://instant.bookingjini.com/widget.bundle.js",
                    async: true
                });
            })();
            </script>
        <!-- End Instant Booking Widget-->
        
        
<script src="https://jiniassist.bookingjini.com/embed.js?company_id=9930"></script>

		
`)

